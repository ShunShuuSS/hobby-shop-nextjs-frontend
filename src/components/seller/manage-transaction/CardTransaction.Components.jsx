...
const CardTransaction = ({ transaction }) => {
  ...
  const [notifCancelSuccess, setNotifCancelSuccess] = useState(false);
  ...
  const cancelTransaction = async () => {
    try {
      ...
      if (cancelTransaction.affectedRows) {
        if (router.pathname !== "/seller/manage-transaction") {
          setTransactionState({
            handleTransaction: true,
          });
        } else {
          setNotifCancelSuccess(true);
        }
      }
    } catch (error) {}
  };
            ...
            <div className={`block m-5`}>
              <div className={`flex justify-between font-bold mb-2`}>
                ...
                <div className={`text-red-600 text-[15px]`}>
                  ...
                  moment(transaction.transaction_date_send_limit).isSameOrAfter(
                    moment()
                  ) ? (
                    <>
                      Batas proses pukul{" "}
                    ...
                    <div className={`flex mb-3 border rounded-md p-1`}>
                      <div className={`w-[7%] mr-2`}>
                        <img
                          src={
                            config.imageApi +
                            product.transaction_product_photo +
                            `_150` +
                            `.webp`
                          }
                          ...
                          >
                            <div className={`flex m-auto text-center`}>
                              {transactionState.loadingTransactionButton ? (
                                ...
                                <>
                                  {transaction.transaction_req_cancel ==
                                  "requested" ? (
                                    <>Terima pembatalan pesanan</>
                                  ) : (
                                    <>
