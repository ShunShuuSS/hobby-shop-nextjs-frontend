...
const ProfilePage = () => {
  ...
  const loadUserPhoto = async () => {
    if (userContext.UserInfo.user_photo !== null) {
      setUserPhoto(
        config.imageApi + userContext.UserInfo.user_photo + `_500` + `.webp`
      );
    } else {
  ...
  const UserDetail = ({ children, index }) => {
    return (
      <div
        className={`flex mobile-s:text-[12px] mobile-l:text-[13px] tablet:text-[14px]`}
      >
        <div
          className={`mobile-s:w-[4.8rem] mobile-l:w-[6rem] tablet:w-[8rem]`}
        >
          {index}
    ...
    <>
      ...
      <ProfileTab>
        <div className={`block w-full h-auto`}>
          <div className={`mobile-s:block tablet:flex`}>
            <div
              className={`flex tablet:mr-5 mobile-s:justify-center tablet:justify-start`}
            >
              <div
                className={`relative w-[14rem] h-[14rem] group outline-style-1 rounded-md bg-white`}
              >
                {userPhoto !== "" ? (
                  <>
                    <img
            ...
            <div
              className={`block w-full h-full relative mobile-s:mt-5 tablet:mt-0`}
            >
          ...
          <div
            className={`mobile-s:block mobile-l:flex mobile-s:justify-between mt-5`}
          >
            <div>
              <Link href={`/profile/reset-password`}>
                <a>
                  <div
                    className={`mobile-s:text-[13px] mobile-s:h-[2rem] tablet:text-base tablet:h-[3rem] tablet:w-[10rem]`}
                  >
                    <button
                      type="button"
            ...
            <div>
              <div className={`mobile-s:mt-5 mobile-l:mt-0`}>
                <button
                  type="button"
                  className={`mobile-s:text-[13px] mobile-s:h-[2rem] mobile-s:w-full tablet:text-base tablet:h-[3rem] tablet:w-[10rem] text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-md px-5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800`}
